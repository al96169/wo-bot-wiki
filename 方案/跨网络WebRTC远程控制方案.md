...
> App 功能迁移已于 2026-09-06 收官（web-debug 全功能对齐 + 云端远控闭环），本节保留需求描述。
...
### Phase 2：授权与信令服务器（→ R00036）✅ 已完成 2026-07-21

- [x] 创建 `wo-bot-signal` 仓库
- [x] 用户帐号系统：JWT 免绑定认证（Logto OIDC）
- [x] 设备云端注册：本地绑定后同步到云端
- [x] WebRTC 信令转发（offer/answer/ICE）
- [x] 机器人 HMAC 鉴权 + 时间戳防重放
- [x] TURN 短期凭证生成（24h 有效）
- [x] CI/CD：ESLint + GitHub Actions 流水线

### Phase 3：跨网络远程连接与远程控制（→ R00037）✅ 已完成 2026-07-21

- [x] 机器人 `signal_client.py`：连接信令服务器 + 接收远程 offer
- [x] 前端远程连接：登录 → 设备列表 → 发起连接
- [x] 连接模式切换 UI（局域网 / 云端远控）
- [x] 远程控制功能验证（视频 + 云台 + 运动 + 语音喊话）
- [x] 断连恢复 + 多种网络拓扑测试（5G/信令/直连对齐）
